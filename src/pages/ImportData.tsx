...
const ImportData = () => {
  ...
  const importAccruals = async (row: any, marketplaceId: string, importBatchId: string) => {
    ...
    const amountBeforeCol = findColumn(["за продажу или возврат", "за прод"]);
    // ВАЖНО: В Excel колонка называется "Итого, ₽" (с символом ₽, НЕ словом "руб"!)
    const totalCol = findColumn(["итого, ₽", "итого,", "итого"]);
