#!/bin/bash
# Batch delete drafted and failed replies via REST API
# VERSION: 2026-01-15-v1

SUPABASE_URL="https://bkmicyguzlwampuindff.supabase.co"
SERVICE_KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJrbWljeWd1emx3YW1wdWluZGZmIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NDY5NTAyMywiZXhwIjoyMDgwMjcxMDIzfQ.F6BnFa-RMYI__r-6bhaLzgZ-7_U-mwvgW_-8fgen0Dk"
MARKETPLACE_ID="84b1d0f5-6750-407c-9b04-28c051972162"

echo "=== Starting batch delete via REST API ==="
echo "Found 7,944 drafted + 21 failed = 7,965 total replies to delete"
echo ""

BATCH_SIZE=50
DRAFTED_DELETED=0
FAILED_DELETED=0

# Delete drafted in batches
echo "Deleting drafted replies in batches of ${BATCH_SIZE}..."
while true; do
  # Get batch of IDs
  IDS=$(curl -s -X GET \
    "${SUPABASE_URL}/rest/v1/replies?marketplace_id=eq.${MARKETPLACE_ID}&status=eq.drafted&deleted_at=is.null&select=id&limit=${BATCH_SIZE}" \
    -H "apikey: ${SERVICE_KEY}" \
    -H "Authorization: Bearer ${SERVICE_KEY}" | jq -r '.[].id')

  if [ -z "$IDS" ]; then
    echo "No more drafted replies to delete"
    break
  fi

  COUNT=$(echo "$IDS" | wc -l)

  # Delete each ID
  for ID in $IDS; do
    curl -s -X PATCH \
      "${SUPABASE_URL}/rest/v1/replies?id=eq.${ID}" \
      -H "apikey: ${SERVICE_KEY}" \
      -H "Authorization: Bearer ${SERVICE_KEY}" \
      -H "Content-Type: application/json" \
      -d "{\"deleted_at\": \"$(date -u +%Y-%m-%dT%H:%M:%S.000Z)\"}" > /dev/null
  done

  DRAFTED_DELETED=$((DRAFTED_DELETED + COUNT))
  echo "Deleted ${COUNT} drafted replies (total: ${DRAFTED_DELETED})"

  sleep 0.5
done

echo ""
echo "Deleting failed replies..."

# Delete failed replies (only 21, can do all at once)
while true; do
  IDS=$(curl -s -X GET \
    "${SUPABASE_URL}/rest/v1/replies?marketplace_id=eq.${MARKETPLACE_ID}&status=eq.failed&deleted_at=is.null&select=id&limit=${BATCH_SIZE}" \
    -H "apikey: ${SERVICE_KEY}" \
    -H "Authorization: Bearer ${SERVICE_KEY}" | jq -r '.[].id')

  if [ -z "$IDS" ]; then
    echo "No more failed replies to delete"
    break
  fi

  COUNT=$(echo "$IDS" | wc -l)

  for ID in $IDS; do
    curl -s -X PATCH \
      "${SUPABASE_URL}/rest/v1/replies?id=eq.${ID}" \
      -H "apikey: ${SERVICE_KEY}" \
      -H "Authorization: Bearer ${SERVICE_KEY}" \
      -H "Content-Type: application/json" \
      -d "{\"deleted_at\": \"$(date -u +%Y-%m-%dT%H:%M:%S.000Z)\"}" > /dev/null
  done

  FAILED_DELETED=$((FAILED_DELETED + COUNT))
  echo "Deleted ${COUNT} failed replies (total: ${FAILED_DELETED})"

  sleep 0.5
done

echo ""
echo "=== Cleanup complete! ==="
echo "Total drafted deleted: ${DRAFTED_DELETED}"
echo "Total failed deleted: ${FAILED_DELETED}"
echo "Grand total: $((DRAFTED_DELETED + FAILED_DELETED))"
